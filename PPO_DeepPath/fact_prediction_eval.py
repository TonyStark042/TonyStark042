...
def bfs_two(e1, e2, path, kb, kb_inv):  # 看e1和e2之间是否存在一个给定的路径 path
    start = 0
    end = len(path)
    left = set()
    right = set()
...
rank_stats_E = [(scores_E[i], test_labels[i]) for i in range(len(test_labels))]
rank_stats_R = [(scores_R[i], test_labels[i]) for i in range(len(test_labels))]
rank_stats_rl = [(scores_rl[i], test_labels[i]) for i in range(len(test_labels))]
